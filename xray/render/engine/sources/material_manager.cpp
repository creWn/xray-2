...
public:
	cl_dt_scaler(float s): 
	  m_scaler( s, s, s, 1.f/m_dtex_range)	
	  {}

// 	void setup(constant const* c)
// 	{
// 		backend::ref().set_constant(c, m_scale, m_scale, m_scale, 1.0f/r__dtex_range);
// 	}
...
void material::_free() const
{
	material* this_ = const_cast<material*>(this);
	DELETE( this_);
}

float4 const* material::get_detail_setup() const
{
	return &m_setup->m_scaler;
}
...
void material_manager::load_raw_files(resources::queries_result& data)
{
	...
	for (u32 i = 0; i < data.size(); ++i)
	{
		...
		memory::chunk_reader	ch_it(ptr.c_ptr(), ptr.size(), memory::chunk_reader::chunk_type_sequential);
