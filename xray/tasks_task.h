...
private:
	...
	void					set_event_for_wait_children	(tasks::event * event);
	tasks::event *			event_for_wait_children		() const { return m_event_wait_for_children; }
...
private:
	...
	intrusive_list<task, task_base, & task_base::m_next_task_in_child_queue, threading::single_threading_policy>	m_children;
	threading::atomic32_type	m_child_counter;
	tasks::event *				m_event_wait_for_children;
