...
typedef intrusive_list<key, key, & key::m_next_key>	command_line_keys_list;
char						s_command_line_keys_buffer			[sizeof(command_line_keys_list)];
...
fixed_string512				s_command_line_error;

static fixed_string512		s_finger_print;
...
struct checker
{
	void operator () (key * const command_line_key, pcstr key_name, pcstr key_value)
	{
		...
		{
			LOG_INFO					(logging::settings("", logging::settings::flags_log_only_user_string | 
															   logging::settings::flags_log_to_console),
...
void   show_help_and_exit ( )
{
	...
	LOG_INFO							(logging::settings("", logging::settings::flags_log_only_user_string |
										...
										"      Finger print info: %s",
										build::calculate_build_id(build::build_date()), build::build_date(), s_finger_print.c_str());
										...
										++it )
	{
		...
		if ( it == keys.begin() || strings::compare(key->category(), previous_category) )
		{
...
unsigned	build_station_build_id	( )
{
	...
	initialized							= true;
	if ( XRAY_SSCANF(xray::command_line::s_finger_print.c_str(), "Build %d", &build_station_build_id) != 1 ) {
