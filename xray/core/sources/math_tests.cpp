...
static void axix_angle_from_random_angles( float3 &axis, float& angle, const float3 &angles )
{
...
static bool angle_similar(  float angle, float to_angle, float epsilon = epsilon_5 )
{
	...
	return similar( angle, to_angle, epsilon );
}
...
static bool similar( const float3 &axis,  float angle, const  float3 &to_axis, float to_angle )
{
	...
	return angle_similar( angle, to_angle ) && similar( test_axis, to_axis ); 
...
static void test_axis_angle_to_quaternion( const float3 &angles )
{
	...
	axix_angle_from_random_angles( axis, angle, angles );

	quaternion test_quaternion0 (  axis, angle );

	float3 test_axis = float3().set(0,0,0);
	float  test_angle = 0;
	test_quaternion0.axe_angle( test_axis, test_angle );

	TEST_ASSERT ( similar( axis, angle, test_axis, test_angle ) );
...
static void test_axis_angle_to_matrix( const float3 &angles )
{
	...
	q.axe_angle( test_axis, test_angle );
...
////////////////////////////////////////////////////////////////////////////////////
	...
	to_close_xyz( test_angles, angles, pi_d2 );
	TEST_ASSERT( math::similar( test_angles, angles ) );
	...
	to_close_xyz( test_angles, angles, pi_d2 );
	TEST_ASSERT( math::similar( angles, test_angles ) );
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	//math::to_close_xyz( cmp1, cmp0, pi_d2  );
	//TEST_ASSERT( math::similar( cmp1, cmp0, pi_d2 ) );
///////////////////////////////////////////////////////////////////////////////////////
