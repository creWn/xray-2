...
static bool parse_item_value(memory::base_allocator* allocator, binary_config_value& itm, pcstr value)
{
	itm.type			= t_string;
	...
	case t_string:
	{
		...
		memory::copy		(const_cast<pvoid>(itm.data), length*sizeof(char), value, length); //inclusive zero
		itm.count			= length;
		break;
	}
	case t_float:
...
void xray::core::configs::parse_string_data(memory::reader F, memory::base_allocator* allocator, pbyte* dest, unsigned& dest_size)
{
	...
	for(; it!=it_e; ++it)
	{
		...
		if(t->type>t_embedded_types)
			XRAY_FREE_IMPL( *allocator, const_cast<pvoid&>(t->data) );
...
xray::core::configs::binary_config* xray::core::configs::create_from_ltx_text(pcstr text, memory::base_allocator* allocator, pcstr fn)
{
	...
	{
		FILE*		f;
		if ( !fs::open_file( &f, fs::open_file_read, fn ) )
			LOG_INFO("unable to open file [%s]", fn);
		else
		{
			fwrite					(binary_buff, binary_buff_size, 1, f);
			fclose					(f);
		}
