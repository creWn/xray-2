...
public:
	...
	inplace_unmanaged_cook	(test_resource_class, reuse_type, create_in_current_thread ? use_current_thread_id : use_cooker_thread_id,
							 allocate_in_current_thread ? use_user_thread_id : use_resource_manager_thread_id),
										 ...
										 mutable_buffer				in_out_unmanaged_resource_buffer)
	{
		...
		in_out_query.save_generated_resource	(generated_file_data);

		fs::path_string					disk_path;
		if ( ! in_out_query.select_disk_path_from_request_path(& disk_path) )
		{
		...
		in_out_query.set_unmanaged_resource	(test_resource, 
											 ...
											 memory_type_non_cacheable_resource : memory_type_managed_resource,
											 ...
											 mutable_buffer				in_out_unmanaged_resource_buffer)
	{
		...
		in_out_query.set_unmanaged_resource			(out_resource, 
													 memory_type_non_cacheable_resource, 
													 in_out_unmanaged_resource_buffer.size());
		in_out_query.finish_query					(result_success);
	}
