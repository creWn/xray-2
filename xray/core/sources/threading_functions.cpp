...
void thread_entry				( pvoid argument )
{
	...
	unsigned const name_for_debugger_length		= strings::length(params_raw->thread_name_for_debugger);
	...
	unsigned const name_for_logging_length		= strings::length(params_raw->thread_name_for_logging);
...
thread_id_type spawn	(
		...
		unsigned const stack_size,
		unsigned const hardware_thread,
		tasks_awareness tasks_awareness,
		pvoid * out_handle
	)
{
	ASSERT								( hardware_thread != unsigned(-1) );
...
static unsigned	*		s_logical_to_physical_core_index	=	NULL;
static unsigned			s_logical_core_count				=	0;

void   initialize_core_affinity	( );

unsigned logical_to_physical_core ( unsigned logical_core )
{
...
{
	unsigned const physical_count				= actual_core_count();
	...
	else if ( g_core_affinity.is_set_as_string(& core_affinity) ) 
	{
		...
		for ( unsigned i=0, n=core_affinity.length(); i<n; ++i )
...
void initialize_core_affinity				( )
{
	...
	fixed_string512							core_affinity;
	if ( g_debug_single_thread ) {
		s_logical_to_physical_core_index	= MT_ALLOC(unsigned, s_logical_core_count);
		s_logical_to_physical_core_index[0]	= 0;
	}
	else if ( g_core_affinity.is_set_as_string(& core_affinity) ) {

		s_logical_to_physical_core_index	= MT_ALLOC(unsigned, s_logical_core_count);
		unsigned current_logical_index			= 0;
		for ( unsigned i=0; i<core_affinity.length(); ++i )
			if ( core_affinity[i] != '0' ) {
...
void set_current_thread_affinity		( unsigned const hardware_thread )
{
	...
	unsigned const previous_value				=	(unsigned)tls_get_value(s_thread_affinity_tls_key);
...
unsigned   current_thread_affinity ()
{
	R_ASSERT									(s_thread_affinity_tls_key, "first call set_current_thread_affinity");
	unsigned hardware_thread						=	(unsigned)tls_get_value(s_thread_affinity_tls_key);
	return										hardware_thread ? (hardware_thread - 1) : unsigned(-1);
}
...
static unsigned	s_thread_logging_name_tls_key	=	tls_get_invalid_key();
