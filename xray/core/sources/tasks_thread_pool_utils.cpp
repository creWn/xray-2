...
command_line::key static		s_spin_count	("spin_count", 
												 "", 
												 "task system", 
								...
								m_user_thread_tls(MT_ALLOC(thread_tls, max_user_threads), max_user_threads)
{
	...
	m_all_task_threads_started.wait				(threading::event::wait_time_infinite);
	m_initialized							=	true;
}
...
void   thread_pool::log_columns_header ()
{
	...
	LOGI_INFO									("tasks", logging::settings(NULL, logging::settings::flags_log_to_console | logging::settings::flags_log_only_user_string),
												"%s", output.c_str());
}
...
void   thread_pool::pause_all_task_threads ()
{
	...
	m_all_task_threads_paused.wait				(threading::event::wait_time_infinite);
...
void   thread_pool::resume_all_task_threads ()
{
	...
	m_all_task_threads_resumed.wait				(threading::event::wait_time_infinite);
