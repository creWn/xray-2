...
class managed_resource_allocator : public resource_allocator, public memory::base_allocator
{
	typedef resource_allocator	super;
...
public:
	...
	virtual	size_t		total_size						() const { return resource_allocator::total_size(); }
	virtual	size_t		allocated_size					() const { return resource_allocator::total_size() - resource_allocator::get_free_size(); }
...
private:
	...
	resource_allocator & get_node_arena					(managed_node *		node);

	void				log_defragmenter_state			() const;
	void				log_defragmenter_nodes			(resource_allocator const & alloc) const;
...
private:
	...
	typedef fixed_vector<unmovable, 32 + max_parallel_with_defragmentation_file_operations_allowed>	unmovables_t;
	...
	resource_allocator									m_temp_arena;
