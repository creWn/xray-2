...
void   thread_pool::on_task_thread_exited (thread_tls * const tls)
{
	...
	threading::mutex_raii raii					(m_thread_exiting_mutex);
...
void   thread_pool::on_current_thread_locks ()
{
	...
	if ( !resulting_value && !zero_tasks() )
	{
											  ...
											  ++it )
		{
			...
			{
				if ( try_activate_task_thread(tls, hardware_thread) )
					break;
			}				
		}
	}
...
void   thread_pool::wait_for_task_list (task * parent)
{
	...
	if ( !parent->no_children_to_execute() )
		parent->event_for_wait_children()->wait	(threading::event::wait_time_infinite);
...
void   thread_tls::thread_proc_impl ()
{
	...
	while ( identity(true) )
	{
		if ( deactivated )
		{
			event_should_work.wait					(threading::event::wait_time_infinite);
		...
		if ( pool->paused() )
		{
			...
			event_pause_ended.wait				(threading::event::wait_time_infinite);
...
{
	event_start_thread_work.wait				(threading::event::wait_time_infinite);
