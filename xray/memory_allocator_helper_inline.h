...
{
	static void		call			( T* const begin, T* const end )
	{
		T* i				= begin;
		for ( ; i != end; ++i )
			new ( i ) T		( );
...
struct call_constructor_helper<T, true>
{
	static void		call			( T* const begin, T* const end )
	{
		XRAY_UNREFERENCED_PARAMETERS	( begin, end );
	}

}; // struct call_constructor_helper

template <class T>
void   call_constructor		( T* const begin, T* const end )
{
#ifdef _MSC_VER
...
void	  delete_array_helper_impl	( A& allocator, 
									  T*& pointer 
...
void   free_helper_impl	( A& allocator, T*& pointer, pvoid const top_pointer XRAY_CORE_DEBUG_PARAMETERS_DECLARATION )
{
...
template < typename A, typename T >
void   free_helper				( A& allocator, T* pointer XRAY_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	if ( !pointer )
		return;
...
template <typename T>
template <typename A>
pvoid   new_helper<T>::call		( A& allocator XRAY_CORE_DEBUG_PARAMETERS_DECLARATION )
{
...
template < typename A, typename T >
void delete_array_helper	( A& allocator, T*& pointer XRAY_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	detail::delete_array_helper_impl	( allocator, pointer XRAY_CORE_DEBUG_PARAMETERS, detail::call_destructor_predicate( ) );
}

template < typename A, typename T, typename P >
void   delete_array_helper	( A& allocator, T*& pointer XRAY_CORE_DEBUG_PARAMETERS_DECLARATION, P const& delete_predicate)
{
...
#	pragma managed (push, on)
	template < typename A, typename T >
	void delete_helper	( A& allocator, T* pointer XRAY_CORE_DEBUG_PARAMETERS_DECLARATION )
	{
		detail::delete_helper_impl		( allocator, pointer XRAY_CORE_DEBUG_PARAMETERS, detail::call_destructor_predicate( ) );
	}
	template < typename A, typename T, typename P >
	void delete_helper		( A& allocator, T* pointer XRAY_CORE_DEBUG_PARAMETERS_DECLARATION, P const& delete_predicate )
	{
...
template <typename A>
pvoid   malloc_helper			( A& allocator, size_t const size, pcstr description XRAY_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	return				allocator.malloc_impl( size, description XRAY_CORE_DEBUG_PARAMETERS );
}

template <typename A>
pvoid   malloc_helper			( A& allocator, size_t const size XRAY_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	return				allocator.malloc_impl( size XRAY_CORE_DEBUG_PARAMETERS );
}

template <typename A, typename T>
pvoid   realloc_helper			( A& allocator, T* pointer, size_t const size, pcstr description XRAY_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	return				allocator.realloc_impl( pointer, size, description XRAY_CORE_DEBUG_PARAMETERS);
}

template <typename A, typename T>
pvoid   realloc_helper			( A& allocator, T* pointer, size_t const size XRAY_CORE_DEBUG_PARAMETERS_DECLARATION )
{
