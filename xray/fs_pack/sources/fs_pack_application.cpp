...
void   application::initialize (u32 argc, pstr const* argv)
{
	...
	xray::core::initialize					( "fs_pack", core::create_log, core::perform_debug_initialization );
}
												...
												++it )
	{
		...
		if ( total_size_value.get_type() == configs::t_string )
		{
			if ( strings::equal((pcstr)total_size_value, "no_limit") )
				item.max_size		=	(u32)fat_inline_data::item::no_limit;
			else
		...
		{
			item.max_size			=	(u32)total_size_value;
		}
...
void   application::execute ()
{
	...
	if ( pack_command )
	{
		...
		if ( inline_config_command.is_set_as_string(& config_file) )
		{
			...
			xray::resources::query_resource_and_wait	(convert_to_portable(query_path.c_str()).c_str(), xray::resources::config_lua_class, & on_inline_config_loaded, & g_fs_pack_allocator);
