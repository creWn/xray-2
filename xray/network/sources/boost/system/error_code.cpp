...
# if defined( BOOST_WINDOWS_API ) && !defined( _XBOX )
#   include <windows.h>
...
#	define NOMINMAX
#	include <Xtl.h>
# endif // # if defined( BOOST_WINDOWS_API ) && !defined( _XBOX )
...
namespace
{
  ...
  xray::network::std_string generic_error_category::message( int ev ) const
  {
    static xray::network::std_string unknown_err( "Unknown error" );
  ...
  #  endif
          ...
          if ( (bp = static_cast<char*>(std::malloc( sz ))) == 0 )
            return xray::network::std_string( "ENOMEM" );
        }
      }
      xray::network::std_string msg;
      try
      {
...
# if !defined( BOOST_WINDOWS_API )
  ...
  xray::network::std_string system_error_category::message( int ev ) const
  {
...
//Chris
  xray::network::std_string system_error_category::message( int ev ) const
  {
# if !defined(BOOST_NO_ANSI_APIS) && !defined(_XBOX)
    LPVOID lpMsgBuf;
    ...
    if (retval == 0)
        return xray::network::std_string("Unknown error");
        
    xray::network::std_string str( static_cast<LPCSTR>(lpMsgBuf) );
    ::LocalFree( lpMsgBuf ); // free the buffer
# elif defined(_XBOX)
	char tmp_buff[64];
	sprintf(tmp_buff, "unknown system error: %08x", ev);
	xray::network::std_string str(tmp_buff);
# else  // WinCE workaround
    ...
    xray::network::std_string str( narrow_buffer );
    ::LocalFree( lpMsgBuf ); // free the buffer
# endif
    while ( str.size()
      && (str[str.size()-1] == '\n' || str[str.size()-1] == '\r') )
