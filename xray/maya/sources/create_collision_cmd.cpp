...
#include <xray/core/sources/math_utils.h>
...
MFnSkinCluster get_skin_cluster(MFnDependencyNode& joint_node, MStatus& stat)
{
...
{ 
	...
	if(argData.isFlagSet(parentFlag))
	{
		...
		MDagPath			parent = get_path_by_name(parent_name, stat);
	...
	if(argData.isFlagSet(typeFlag))
	{
		...
		if(argData.isFlagSet(autoFlag))
		{
			...
			MDagPath joint_path = get_path_by_name(joint_name, stat);
			CHK_STAT_R			(stat);
			MFnIkJoint			jointFn(joint_path.node(), &stat);
			CHK_STAT_R			(stat);
			MFnSkinCluster skin_cluster = get_skin_cluster(jointFn, stat);
...
/*
			MPointArray				points;
			itPoints.allPositions	(points, MSpace::kWorld);

			u32 l1				= points.length();
			u32 l2				= weights_list.length();
*/
