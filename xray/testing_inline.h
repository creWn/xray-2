...
suite_base<derived>::suite_base ()
{
	detail::on_new_suite();
}

template <class derived>
bool   suite_base<derived>::run_tests ()
{
	return	detail::run_tests_impl (singleton()->m_tests.pop_all_and_clear(), typeid(derived).name());
}

template <class derived>
void   suite_base<derived>::add_test (test_base* const test)
{
...
template <class derived>
derived *   suite_base<derived>::singleton ()
{
	if ( !s_suite )
	{
