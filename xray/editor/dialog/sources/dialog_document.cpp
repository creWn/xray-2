...
void dialog_document::load()
{
	...
	dialog_path	+= unmanaged_string(this->Name).c_str();
	dialog_path	+= ".resource";

	query_result_delegate* rq = NEW(query_result_delegate)(gcnew query_result_delegate::Delegate(this, &dialog_document::on_resources_loaded));
...
void dialog_document::on_resources_loaded(xray::resources::queries_result& data)
{
	...
	if(data.size() > 0)
	{
		*m_resources = static_cast_checked<dialog_resources*>(data[0].get_unmanaged_resource().c_ptr());
...
void dialog_document::load_impl(dialog* d, dialog_graph_node_layout* l)
{
	...
	const vector<dialog_node_base_ptr>* nodes_vec = d->get_phrases();
	vector<dialog_node_base_ptr>::const_iterator i = nodes_vec->begin();
	for(; i!=nodes_vec->end(); ++i)
	{
		dialog_graph_node^ n = make_node();
		n->object = i->c_ptr();
	...
	for(; i!=nodes_vec->end(); ++i)
	{
		...
		{
			dialog_graph_node^ n1 = m_hypergraph->find_node_by_id(nds, (*i)->id()); 
			dialog_graph_node^ n2 = m_hypergraph->find_node_by_id(nds, cur_dlg_link->child()->id()); 
			if(n1 && n2)
			{
...
void dialog_document::on_hypergraph_click(System::Object^ , System::EventArgs^ e)
{
	...
	if(safe_cast<System::Windows::Forms::MouseEventArgs^>(e)->Button==System::Windows::Forms::MouseButtons::Left)
	{
		...
		if(l->Count!=0)
			m_editor->show_properties(l[l->Count-1]);
...
void dialog_document::copy(bool del)
{
	...
	safe_cast<dialog_files_view_panel^>(get_editor()->multidocument_base->view_panel)->copied_node = nullptr;
	configs::lua_config_ptr cfg = configs::create_lua_config();
	xray::strings::stream stream(g_allocator);

	dialog* tmp_dialog = NEW(dialog)();
	dialog_graph_node_layout* tmp_layout = NEW(dialog_graph_node_layout)();
	for each(dialog_graph_node^ n in nds)
	{
...
{
	if(safe_cast<dialog_files_view_panel^>(get_editor()->multidocument_base->view_panel)->copied_node!=nullptr)
	{
		System::String^ node_name = safe_cast<dialog_files_view_panel^>(get_editor()->multidocument_base->view_panel)->copied_node->FullPath;
...
void dialog_document::del()
{
	...
	if(m_hypergraph->selected_nodes->Count!=0 || tmp_links->Count!=0)
		m_hypergraph->remove_group(m_hypergraph->selected_nodes, tmp_links);
	m_hypergraph->selected_links->Clear();
	m_hypergraph->set_selected_node((nodes^)nullptr);
}

void dialog_document::on_node_double_click(dialog_graph_node^ n)
{
	if(n->is_dialog())
		get_editor()->multidocument_base->load_document(n->text);
	else
		get_editor()->show_text_editor(n);
}
...
void dialog_document::on_hypergraph_selection_changed()
{
	...
	{
		dialog_graph_node^ n = safe_cast<dialog_graph_node^>(m_hypergraph->selected_nodes[m_hypergraph->selected_nodes->Count-1]);
...
void dialog_document::on_node_property_changed(dialog_graph_node^ n, String^ l, Object^ new_val, Object^ old_val)
{
	m_hypergraph->on_node_property_changed(n, l, new_val, old_val);
}

void dialog_document::on_node_text_changed(dialog_graph_node^ n, String^ new_str, String^ new_text, String^ old_str, String^ old_text)
{
