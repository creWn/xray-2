...
public:
				...
				mutable_buffer	(pvoid data, unsigned size) : m_data((pstr)data), m_size(size) {}

				template <class T>
				mutable_buffer	(T * data, unsigned size) : m_data((pstr)data), m_size(size) 
				{
	...
	void		operator +=		(unsigned offs);
	operator	bool			() const	{ return !!m_data; }
	bool		operator ==		(mutable_buffer const & other) const;

	char &		operator []		(unsigned index)			{ ASSERT(index < m_size); return m_data[index]; }
	char		operator []		(unsigned index) const	{ ASSERT(index < m_size); return m_data[index]; }

	static mutable_buffer	zero	() { return mutable_buffer(NULL, 0); }

	mutable_buffer	slice		(unsigned offs, unsigned count) const;
...
public:
				...
				const_buffer	(pcvoid data, unsigned size) : m_data((pcstr)data), m_size(size) {}
				template <class T>
				const_buffer	(T const * data, unsigned size) : m_data((pcstr)data), m_size(size) 
				{
	...
	const_buffer	slice		(unsigned offs, unsigned count) const;
	void		reset			() { m_data = NULL; m_size = 0; }
...
template <class T>
inline mutable_buffer buffer	(T* data, unsigned size);

template <class T>
inline const_buffer	buffer		(T const* data, unsigned size);
